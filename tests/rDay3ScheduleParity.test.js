...
// (migrations/2026-08-16_r_day3_business_day_intake_authority.sql) restate
// src/schedule/serviceSchedule.js's DEFAULT_SCHEDULE constants in raw SQL,
// because a PL/pgSQL trigger cannot call into Node. This is the drift-risk
// point the R-DAY3 amendment names as a hard gate ("If exact parity cannot
// be guaranteed: STOP before cutover").
//
...
//      (240/480/1050/1080) match DEFAULT_SCHEDULE's minutes exactly, so a
...
function sqlMirror(minutesOfDay, dateStr) {
  ...
  const canCreateNewOrder = (minutesOfDay >= 480 && minutesOfDay < 1050) || (minutesOfDay >= 1080);
...
const SQL = fs.readFileSync(
  path.join(__dirname, "..", "migrations", "2026-08-16_r_day3_business_day_intake_authority.sql"),
  "utf8",
);
...
assert("B2: lunchEnsureStartMin (08:00=480) appears as the canCreateNewOrder lower bound",
  DEFAULT_SCHEDULE.lunchEnsureStartMin === 480 && /v_minutes_of_day >= 480/.test(SQL));
assert("B3: lunchBoundaryMin (17:30=1050) appears as both the PRANZO/SERA split and the buffer start",
  DEFAULT_SCHEDULE.lunchBoundaryMin === 1050 && (SQL.match(/1050/g) || []).length >= 2);
assert("B4: dinnerEnsureStartMin (18:00=1080) appears as the buffer end / SERA_WINDOW start",
  DEFAULT_SCHEDULE.dinnerEnsureStartMin === 1080 && /v_minutes_of_day >= 1080/.test(SQL));
assert("B5: both resolve_order_intake_context_v1 and get_order_intake_context_v1 contain the full constant set",
  (SQL.match(/v_minutes_of_day < 240/g) || []).length === 2
  && (SQL.match(/v_minutes_of_day >= 480/g) || []).length === 2
  && (SQL.match(/v_minutes_of_day >= 1080/g) || []).length === 2);

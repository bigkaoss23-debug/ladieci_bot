...
const ALLOWLIST = {
  ...
  'src/delivery/giroIntentReconciler.js': { category: 'NOT_GIRO_SEMANTICS', reason: 'W5 Intent Activation: one comment-only mention ("never performs a raw giro_members/manual_giros/ordenes.manual_giro_id write"); the module itself only calls giro_authority_list_pending_intents_v1/giro_authority_consume_intent_v1 via sbRpc, reads/writes nothing raw' },
  ...
  'src/core/delivery/tripOperationalState.js': { category: 'NOT_GIRO_SEMANTICS', reason: 'Planner W6.6 Trip Operational HTTP wire bridge; the match is its own header comment naming the raw/legacy facts it explicitly does NOT read (proven executable-code-clean by tests/tripOperationalState.test.js, which strips comments before scanning)' },

...
// Proves the end-of-service reset is transactional (reset_rider_state_if_idle), refuses to
...
const resetBody = rpc.slice(rpc.indexOf("FUNCTION public.reset_rider_state_if_idle"), rpc.indexOf("Privileges:"));
...
(async () => {
  let r = await riderTrip.resetIfIdle();
  check("resetIfIdle -> reset_rider_state_if_idle, 200 on OK", RPC_RESULT._fn === "reset_rider_state_if_idle" && r.status === 200);
  RPC_RESULT = { httpStatus: 200, ok: true, body: { ok: false, code: "ACTIVE_TRIP_CONFLICT" } };
  r = await riderTrip.resetIfIdle();

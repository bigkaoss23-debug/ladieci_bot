...
(async () => {
  ...
  section("T3 — createOrden: 00:10 / durata 12 → forno_out 23:58, no clamp");
  {
    ...
    // Ignora i campi ISO-datetime (updated_at, *_at): non sono orari "di giornata"
    // e contengono ":2X:" a seconda del wall-clock → falso positivo.
    assert("nessun 24/25", !/\b2[4-9]:\d\d/.test(JSON.stringify(row).replace(/\d{4}-\d\d-\d\dT[\d:.]+Z?/g, "")), JSON.stringify(row));
  }
  ...
  section("T5 — updateOrden: cambio hora 00:10 → forno_out 23:58");
  {
    ...
    assert("nessun 24/25", !/\b2[4-9]:\d\d/.test(JSON.stringify(u.patch).replace(/\d{4}-\d\d-\d\dT[\d:.]+Z?/g, "")), JSON.stringify(u.patch));
  }

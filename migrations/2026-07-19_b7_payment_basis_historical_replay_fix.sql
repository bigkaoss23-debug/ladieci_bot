-- migrations/2026-07-19_b7_payment_basis_historical_replay_fix.sql
-- Access Control V2 / B7A2E — preserve payment-basis historical replay.
-- TARGET PROJECT REF: tdikhfeinufaahagmpjz   ***STAGING ONLY***
--
-- Fixes the guarded payment-basis replay path only. The current B7A2D definitions
-- correctly check same-scope idempotency before generic already-paid rejection, but
-- rebuild the replay digest from mutable public.ordenes snapshots. If the order is
-- later voided/refunded, the replay digest can drift from the immutable basis event
-- and raise AUTH_IDEMPOTENCY_CONFLICT. Replays must instead rebuild their digest from
-- the existing event's immutable amount/estado/pay-state snapshots plus the current
-- normalized request semantics. Fresh insert semantics remain unchanged.
--
-- Replaces only order_mark_paid and order_import_legacy_payment guarded signatures.
-- No data migration, no ledger/order/auth mutation, no flags, no grants/RLS/table changes.
BEGIN;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version='20260710075612')
  THEN RAISE EXCEPTION 'B7A2E refused: staging sentinel migration absent — wrong database?'; END IF;
END $$;

-- migrations/2026-08-23_o2_open_service_cross_midnight_continuity.ROLLBACK.sql
-- Reverts O-2 by restoring resolve_order_intake_context_v1 and
-- get_order_intake_context_v1's pre-O-2 (post-O-1) bodies byte-identical:
-- the 00:00-08:00 floor blocks new-order creation again unconditionally,
-- including for an already-open, same-business-date Operational Service.
--
-- READ THIS BEFORE RUNNING IT. Rolling back reintroduces the cross-midnight
-- continuity gap this migration closed: a service legitimately open and
-- trading since before midnight will start refusing new comandas the
-- instant the clock crosses 00:00, even though nothing about the service
-- itself changed. No table, column, trigger or grant changes in either
-- direction; this file changes two function bodies only.
...
BEGIN
  ...
  IF v_resolve_src IS NULL OR v_resolve_src LIKE '%v_continuity%' THEN
    RAISE EXCEPTION 'O-2 rollback post-condition failed: the pre-O-2 body was not restored in resolve_order_intake_context_v1';
  END IF;
  ...
  IF v_get_src IS NULL OR v_get_src LIKE '%hasValidCurrentService%' THEN
    RAISE EXCEPTION 'O-2 rollback post-condition failed: the pre-O-2 body was not restored in get_order_intake_context_v1';
  END IF;
END $$;

-- Rollback for P0-B mesa_post_payment_v1 orphaned-order fix. Restores the
-- exact pre-fix body (byte-identical to 2026-08-02_v3j_mesa_nomenclature_
-- cutover.sql's version) -- payments keep working exactly as before, but
-- closing a table with non-terminal orders once again leaves them stranded.
-- Does not touch any row this migration's forward file already wrote
-- (orden_estado_logs rows and any ordenes.estado='CHIUSO_FORZATO' this fix
-- produced remain -- this is a schema/behavior revert, not a data undo).
BEGIN;
...
BEGIN
  IF to_regprocedure('public.mesa_post_payment_v1(uuid,text,text,uuid,text,text,text,text,numeric,integer,uuid[],jsonb)') IS NULL THEN
    RAISE EXCEPTION 'P0-B rollback refused: mesa_post_payment_v1 not found';
  END IF;
END $$;

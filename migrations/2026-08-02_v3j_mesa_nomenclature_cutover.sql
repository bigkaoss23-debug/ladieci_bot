-- Access Control V3 -- V3-J: rename the runtime "messa_*" contract to "mesa_*".
-- TARGET PROJECT REF: tdikhfeinufaahagmpjz   ***STAGING ONLY***
-- DRAFT ONLY: NOT applied to any database by this change.
--
-- Pure nomenclature cutover -- no behavior change. "Messa" (double-S) was a
-- typo of the Spanish "mesa" (table); it was already corrected across the
-- JavaScript layer (file paths, identifiers, folders) but intentionally left
-- in the live Postgres contract because renaming a wire vocabulary shared by
-- triggers, RPCs and two deployed apps needs its own coordinated migration +
-- deploy -- see the comments this replaces in mesaService.js/mesaApi.js.
--
-- Every function body below is byte-identical in LOGIC to its "messa_*"
-- predecessor (v3h/v3h.1/v3h.1a/v3h.2/v3i); only identifiers change:
--   - function/trigger names:  messa_*_v1        -> mesa_*_v1
--   - error codes:             MESSA_*           -> MESA_*
--   - idempotency scope tag:   'messa_' || ...   -> 'mesa_' || ...
--   - ledger meta tag:         'source':'messa'  -> 'source':'mesa'
--
-- Coordinated deploy order (see MIGRATION_MANIFEST.md checklist added by this
-- same slice): this migration must be the ONLY thing between two backend
-- deploys that (1) still call messa_* and (2) call mesa_* -- see the
-- "V3-J cutover ordering" section of the manifest.
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'V3-J refused: staging sentinel migration absent -- wrong database?';
  END IF;
  -- Hard prerequisite: V3-I (covers-deferred) must already be live, since the
  -- function bodies below are V3-I's amended versions, not V3-H's originals.
  IF to_regprocedure('public.messa_release_empty_session_v1(uuid,text,uuid)') IS NULL
     OR to_regprocedure('public.messa_guard_covers_monotonic_v1()') IS NULL
     OR to_regprocedure('public.messa_open_reservation_v1(uuid,text,uuid,integer,uuid)') IS NULL
     OR to_regprocedure('public.messa_save_reservation_v1(uuid,text,uuid,uuid,text,text,integer,text,text,text,integer)') IS NULL
  THEN
    RAISE EXCEPTION 'V3-J refused: exact V3-H.2/V3-I predecessor not found -- apply V3-I first';
  END IF;
END $$;

-- ══════════════════════════════════════════════════════════════════════════
-- 1) Create every mesa_* function (CREATE OR REPLACE -> safe to re-run)
-- ══════════════════════════════════════════════════════════════════════════

-- ── Atomic order preparation + first-comanda covers capture (from V3-I) ────
...
-- ── Open: covers optional/nullable (from V3-I) ─────────────────────────────
CREATE OR REPLACE FUNCTION public.mesa_open_session_v1(
  p_workspace_id uuid,
  p_by_actor text,
  p_table_id uuid,
...
BEGIN
  ...
  -- The FOR UPDATE lookup above closes the common sequential-reopen race, but two
  -- truly simultaneous first-opens on a table with no existing row yet race past
  -- it together; the partial unique index is the final arbiter. Map its violation
  -- to the same clean domain error instead of surfacing a raw constraint message.
  BEGIN
...
CREATE OR REPLACE FUNCTION public.mesa_save_table_v1(
  p_workspace_id uuid,
  p_by_actor text,
  p_table_id uuid,
...
-- Payment: refuses to run before covers are real (from V3-I) --
...
-- Undo an accidental open (from V3-I) --
CREATE OR REPLACE FUNCTION public.mesa_release_empty_session_v1(
  p_workspace_id uuid,
  p_by_actor text,
  p_table_session_id uuid
)
...
-- ── Reservations (from V3-H.2) ──────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.mesa_save_reservation_v1(
  p_workspace_id uuid,
  p_by_actor text,
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE EXCEPTION 'MESA_RESERVATION_TIME_INVALID' USING ERRCODE='22023';
  END;
  ...
  ) THEN RAISE EXCEPTION 'MESA_RESERVATION_FORBIDDEN' USING ERRCODE='42501'; END IF;
  ...
  IF v_table.capacity IS NOT NULL AND p_covers_total > v_table.capacity THEN
    RAISE EXCEPTION 'MESA_RESERVATION_CAPACITY_EXCEEDED' USING ERRCODE='23514';
  END IF;
  ...
  ) THEN RAISE EXCEPTION 'MESA_RESERVATION_OVERLAP' USING ERRCODE='23P01'; END IF;
...
CREATE OR REPLACE FUNCTION public.mesa_set_reservation_status_v1(
  p_workspace_id uuid,
  p_by_actor text,
  p_reservation_id uuid,
...
CREATE OR REPLACE FUNCTION public.mesa_open_reservation_v1(
  p_workspace_id uuid,
  p_by_actor text,
  p_reservation_id uuid,
  p_expected_version integer,
...
-- ── Two service-close guards raise the same renamed code (their own function
-- names never carried "messa"/"mesa" -- only the raised string changes) ────
...
-- ══════════════════════════════════════════════════════════════════════════
-- 2) Repoint every trigger at its new mesa_* function
-- ══════════════════════════════════════════════════════════════════════════

DROP TRIGGER IF EXISTS messa_prepare_table_order_v1 ON public.ordenes;
CREATE TRIGGER mesa_prepare_table_order_v1
...
CREATE TRIGGER mesa_snapshot_order_lines_v1
AFTER INSERT ON public.ordenes
FOR EACH ROW EXECUTE FUNCTION public.mesa_snapshot_order_lines_v1();
...
FOR EACH ROW EXECUTE FUNCTION public.mesa_guard_covers_monotonic_v1();

DROP TRIGGER IF EXISTS messa_complete_reservation_v1 ON public.table_sessions;
CREATE TRIGGER mesa_complete_reservation_v1
AFTER UPDATE OF status ON public.table_sessions
FOR EACH ROW EXECUTE FUNCTION public.mesa_complete_reservation_v1();

DROP TRIGGER IF EXISTS messa_guard_reserved_table_v1 ON public.restaurant_tables;
CREATE TRIGGER mesa_guard_reserved_table_v1
BEFORE UPDATE OF active ON public.restaurant_tables
FOR EACH ROW EXECUTE FUNCTION public.mesa_guard_reserved_table_v1();

-- service_sessions_closed_live_work_guard already points at
-- guard_service_session_closed_v1 by name -- CREATE OR REPLACE above updated
-- its body in place; no trigger swap needed.

-- ══════════════════════════════════════════════════════════════════════════
-- 3) Drop every old messa_* function (now trigger-free)
-- ══════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════
-- 4) Grants: revoke from anon/authenticated, execute-only for service_role
-- ══════════════════════════════════════════════════════════════════════════

REVOKE ALL ON FUNCTION public.mesa_prepare_table_order_v1() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_snapshot_order_lines_v1() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_append_only_v1() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_open_session_v1(uuid,text,uuid,uuid,integer) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_save_table_v1(uuid,text,uuid,integer,text,integer,numeric,numeric,text,boolean) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_post_payment_v1(uuid,text,text,uuid,text,text,text,text,numeric,integer,uuid[],jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_release_empty_session_v1(uuid,text,uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_guard_covers_monotonic_v1() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_save_reservation_v1(uuid,text,uuid,uuid,text,text,integer,text,text,text,integer) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_set_reservation_status_v1(uuid,text,uuid,integer,text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_open_reservation_v1(uuid,text,uuid,integer,uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_complete_reservation_v1() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.mesa_guard_reserved_table_v1() FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.mesa_prepare_table_order_v1() TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_snapshot_order_lines_v1() TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_append_only_v1() TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_open_session_v1(uuid,text,uuid,uuid,integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_save_table_v1(uuid,text,uuid,integer,text,integer,numeric,numeric,text,boolean) TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_post_payment_v1(uuid,text,text,uuid,text,text,text,text,numeric,integer,uuid[],jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_release_empty_session_v1(uuid,text,uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_guard_covers_monotonic_v1() TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_save_reservation_v1(uuid,text,uuid,uuid,text,text,integer,text,text,text,integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_set_reservation_status_v1(uuid,text,uuid,integer,text) TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_open_reservation_v1(uuid,text,uuid,integer,uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_complete_reservation_v1() TO service_role;
GRANT EXECUTE ON FUNCTION public.mesa_guard_reserved_table_v1() TO service_role;

...
| 54 | 2026-08-08 → **SERVICE CLOSEOUT V2 / SLICE 2 — POST-CLOSE FINANCIAL RESOLUTION** | 2026-08-08_service_closeout_post_close_financial_resolutions.sql | pending | 2026-08-08 | c0f02d2f8e3cd485 |  <!-- DRAFT — NOT APPLIED to any database. Implements the design docs/SERVICE_CLOSEOUT_V2_SLICE_2_FINANCIAL_RESOLUTION_DESIGN_NOTE.md sketched during Slice 1. Pure addition: creates archived_order_financial_resolutions, an append-only ledger recording what happened to an archived (storico) order's unpaid balance AFTER service close — recovered_payment, write_off, or a full reversal of one specific prior event in the same lineage ('correction' was investigated and deliberately folded into reversal+a fresh event rather than kept as a redundant 4th vocabulary term). Never rewrites storico, serata_summary, order_financial_events, or service_incidents.financial_exposure_cents — those stay frozen "truth at close" forever; this table only layers new append-only facts on top, read separately at report time. Lineage identity is the PAIR (service_session_id, archived_order_id) — NEVER archived_order_id alone, since storico's own uniqueness is UNIQUE(service_session_id, orden_id) and order numbers are reused across sessions/days. Idempotent on action_correlation_id (never order_id+amount — the same amount can legitimately be recovered twice across separate partial payments); a pg_advisory_xact_lock keyed on the lineage pair serializes concurrent writers, including the very first event. original_exposure_cents is captured once per lineage and frozen (ORIGINAL_EXPOSURE_MISMATCH rejects drift); remaining_exposure_cents is computed server-side per event; recovered_payment/write_off fail closed with OVER_RESOLUTION_EXCEEDS_REMAINING rather than clamp; reversal is full-amount-only and a given event can be reversed at most once. created_at uses clock_timestamp(), not now() — real-Postgres validation caught that now()/transaction_timestamp() is frozen for the whole transaction, which let two events on one lineage inserted in the same transaction tie on created_at and made the "latest event" lookup fall back to comparing random uuids; clock_timestamp() advances per statement and fixed it. Integer cents throughout (never numeric/float) — a deliberate, documented unit difference from order_financial_events' decimal-euro amount column, never silently unified here. The one RPC (create_archived_order_financial_resolution) is SECURITY INVOKER, RLS-enabled with zero policies, service_role-only SELECT+INSERT grants (REVOKE ALL FROM service_role first, per the Slice 1.3 deterministic-privilege-floor discipline, applied from the start on this table), and enforces role='admin' server-side as defense-in-depth, independent of any caller claim (same Slice 1.1 trust boundary — not real authentication, no public HTTP path exists yet). archived_order_id carries no foreign key (same FK-less historical-identity philosophy as order_financial_events.order_id/service_incidents.order_id). Nothing here is called by chiudiServizio, order_mark_paid/order_refund/order_void, or any HTTP action — storage/RPC/idempotency/arithmetic foundation only, zero change to any existing financial RPC or service-session rollover behaviour. -->

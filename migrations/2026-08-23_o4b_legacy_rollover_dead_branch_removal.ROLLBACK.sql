-- migrations/2026-08-23_o4b_legacy_rollover_dead_branch_removal.ROLLBACK.sql
-- Reverts O-4.1 by restoring resolve_order_intake_context_v1's pre-O-4.1
-- (post-O-4) body byte-identical: the legacy-rollover UPDATE returns to the
-- slow-path ELSE branch.
--
-- READ THIS BEFORE RUNNING IT. Rolling back does NOT reintroduce any real
-- behaviour: the UPDATE this restores was already proven structurally
-- unreachable before O-4.1 deleted it (every open/closing row has
-- lifecycle_semantics='operational_service_v1' by construction, so the
-- continuity fast-path above it always returns first), so this rollback is
-- a pure code-shape reversal, not a behavioural one. It exists for
-- exact-history reversibility. No table, column, trigger or grant changes
-- in either direction; this file changes one function body.
...
BEGIN
  ...
  ELSE
    ...
    IF v_had_open_or_closing THEN
      -- O-4 (was F-10B) — structurally unreachable: any operational_service_v1
      -- row already returned above. Left in place, not deleted, as the
      -- pre-O-4.1 shape this rollback restores.
...
BEGIN
  ...
  IF v_resolve_src IS NULL OR v_resolve_src NOT LIKE '%rolled_over%' THEN
    RAISE EXCEPTION 'O-4.1 rollback post-condition failed: the legacy-rollover UPDATE was not restored in resolve_order_intake_context_v1';
  END IF;
END $$;

-- Access Control V3 -- V3-I: defer Mesa covers to the first comanda.
-- TARGET PROJECT REF: tdikhfeinufaahagmpjz   ***STAGING ONLY***
-- DRAFT ONLY: not applied to any remote database by this change. Apply manually
-- to staging only after the paired backend deploy, same discipline as V3-H.
--
-- Product decision: covers are unknown when a walk-in table is opened (the guest
-- hasn't been seated with a headcount yet). Asking for them at open time forced an
-- invented number (table capacity, or a "1" default). Real covers are only known
-- once the operator takes the first order, so covers_total now starts NULL and is
-- set atomically by the very first comanda -- never by a separate call that could
-- register covers while the comanda itself fails.
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'V3-I refused: staging sentinel migration absent -- wrong database?';
  END IF;
  IF to_regclass('public.table_sessions') IS NULL
     ...
     OR to_regprocedure('public.messa_post_payment_v1(uuid,text,text,uuid,text,text,text,text,numeric,integer,uuid[],jsonb)') IS NULL
     OR to_regprocedure('public.messa_open_reservation_v1(uuid,text,uuid,integer,uuid)') IS NULL
  THEN
    RAISE EXCEPTION 'V3-I refused: exact V3-H/V3-H.2 predecessor not found';
  END IF;
END $$;

-- ── covers_total becomes deferred, not mandatory at open ───────────────────
ALTER TABLE public.table_sessions ALTER COLUMN covers_total DROP NOT NULL;
ALTER TABLE public.table_sessions DROP CONSTRAINT IF EXISTS table_sessions_covers_total_chk;
ALTER TABLE public.table_sessions
  ADD CONSTRAINT table_sessions_covers_total_chk
    CHECK (covers_total IS NULL OR covers_total BETWEEN 1 AND 99);

COMMENT ON COLUMN public.table_sessions.covers_total IS
  'NULL only between opening a walk-in Mesa and its first comanda (which sets it
   atomically, in the same INSERT transaction, via messa_prepare_table_order_v1).
   A reservation-seated session sets it immediately from the reservation''s real
   guest count. Once non-NULL it can only stay the same or grow (see the monotonic
   guard trigger below); payment is refused while it is still NULL.';

-- Once real covers are registered, they are a fact, not a draft: guard against
-- ever un-setting them or silently shrinking them.
...
-- ── One-time input column for the first comanda ────────────────────────────
-- Ephemeral by construction: messa_prepare_table_order_v1 always nulls it out on
-- NEW before the row is actually written, so it is never persisted with a value.
-- It exists only so the atomic order-insert trigger can receive the real covers
-- count from the same INSERT that creates the first comanda -- no second endpoint,
-- no window where covers are registered but the comanda itself could still fail.
ALTER TABLE public.ordenes
  ADD COLUMN table_covers_total_input integer NULL
    CHECK (table_covers_total_input IS NULL OR table_covers_total_input BETWEEN 1 AND 99);

-- ── Atomic order preparation, extended to also set covers on the first comanda ──
...
-- ── Payment refuses to run before covers are real ──────────────────────────
...
-- ── Open no longer requires (or even accepts a meaningful) covers value ─────
...
-- ── Undo an accidental open: only ever possible while covers are still NULL,
-- which by construction means the session has zero comande (the only writer of
-- covers_total is the first-comanda trigger above). No payment is involved
-- because there is nothing to charge yet.
...
-- ── RLS/grants for the new function; existing ones are unchanged by REPLACE ──

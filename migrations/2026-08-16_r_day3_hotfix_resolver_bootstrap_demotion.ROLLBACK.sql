-- migrations/2026-08-16_r_day3_hotfix_resolver_bootstrap_demotion.ROLLBACK.sql
-- Paired rollback for 2026-08-16_r_day3_hotfix_resolver_bootstrap_demotion.sql.
-- Restores resolve_order_intake_context_v1() to its exact byte-captured
-- pre-hotfix body (the original R-DAY3 migration's own version, which keys
-- demotion on the pointer's current_period_id rather than the actual
-- globally active session). This reintroduces the bootstrap defect this
-- hotfix fixes -- only ever use this to fully unwind BOTH the hotfix and
-- the original R-DAY3 migration together, never in isolation.
BEGIN;
...
BEGIN
  ...
  ) THEN RAISE EXCEPTION 'R-DAY3 hotfix rollback refused: resolver does not contain the hotfixed body -- hotfix was never applied or already rolled back'; END IF;
END $$;
...
BEGIN
  IF (SELECT count(*) FROM public.payment_transactions) <> 20 THEN
    RAISE EXCEPTION 'R-DAY3 hotfix rollback post-condition failed: payment_transactions population changed -- must be exactly 20';
  END IF;
END $$;
